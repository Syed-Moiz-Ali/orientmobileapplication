...
final advisorDashboardProvider = Provider<AdvisorStatsEntity>((ref) {
  ...
  try {
    ...
    return const AdvisorStatsEntity(
      newJobCardsToday: 0, inspectionsToday: 0, pendingApprovals: 0,
      vehiclesWaiting: 0, readyForDelivery: 0, totalOpenJobCards: 0,
    );
  }
});
...
final advisorRecentJobCardsProvider = Provider<List<JobCardEntity>>((ref) {
  ...
  final box = Hive.box<dynamic>('inspections');
  final allData = box.values.whereType<Map>().map((m) => Map<String, dynamic>.from(m)).toList();
  final filtered = allData.where((m) => m['type'] == 'vehicle_customer').toList();
  if (filtered.isEmpty) {
    return const [
      JobCardEntity(id: 'JC-2024-089', customerName: 'Ahmed Hassan', vehicleInfo: 'Toyota Camry', time: '09:15 AM', createdDate: '12/07/2024 09:15', lastUpdated: '12/07/2024 09:15', status: JobCardStatus.inProgress),
      JobCardEntity(id: 'JC-2024-088', customerName: 'Fatima Ali', vehicleInfo: 'Honda Accord', time: '08:45 AM', createdDate: '12/07/2024 08:45', lastUpdated: '12/07/2024 08:45', status: JobCardStatus.pendingApproval),
      JobCardEntity(id: 'JC-2024-087', customerName: 'Khalid Rashid', vehicleInfo: 'Nissan Patrol', time: '08:00 AM', createdDate: '12/07/2024 08:00', lastUpdated: '12/07/2024 08:00', status: JobCardStatus.qualityCheck),
    ];
  }
...
final advisorPendingApprovalsProvider = Provider<List<PendingApprovalEntity>>((ref) {
  ref.watch(advisorRefreshProvider);
  try {
    final box = Hive.box<dynamic>('inspections');
    final allData = box.values.whereType<Map>().map((m) => Map<String, dynamic>.from(m)).toList();
    final approvals = allData
        .where((m) => m['estimateId'] != null && (m['action'] == 'approved' || m['action'] == 'rejected'))
...
final advisorFollowupRemindersProvider = Provider<List<FollowupReminderEntity>>((ref) {
  return const [
...
final advisorInfoProvider = Provider<AdvisorInfo>((ref) {
  return const AdvisorInfo(
    name: 'Ali Rahman', id: 'ADV001',
    branch: 'Main Branch - Dubai', shift: 'Morning (8:00 AM - 5:00 PM)',
  );
});
