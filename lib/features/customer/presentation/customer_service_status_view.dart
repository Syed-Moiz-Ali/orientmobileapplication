      ...
      body: SafeArea(
        ...
        child: Column(
          children: [
            InnerTopBar(
              title: 'Job Tracker',
              trailing: StatusPill(label: '\u25cf Live', bg: AppColors.successBg, fg: AppColors.success),
            ),
            const Divider(height: 1),
            Expanded(
              child: SingleChildScrollView(
                padding: const EdgeInsets.fromLTRB(AppDimensions.s18, AppDimensions.s18, AppDimensions.s18, AppDimensions.s32),
                child: Column(children: [
                  ...
                  AppCard(
                    child: Row(children: [
                      ...
                      _IconBtn(icon: Icons.call_rounded, color: AppColors.success, bg: AppColors.successBg, onTap: () {}),
                      const SizedBox(width: AppDimensions.s8),
                      _IconBtn(icon: Icons.chat_bubble_outline_rounded, color: AppColors.primary, bg: AppColors.primaryBg, onTap: () {}),
                    ]),
                  ),
                  const SizedBox(height: AppDimensions.s14),
                  ...
                  Container(
                    ...
                    child: const Row(children: [
                      Icon(Icons.sync_rounded, color: AppColors.success, size: 17),
...
class _IconBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      width: 36, height: 36,
      decoration: BoxDecoration(color: bg, borderRadius: BorderRadius.circular(AppDimensions.r10)),
...
class _StageItem extends StatelessWidget {
  final ServiceStageEntity stage; final bool isLast;
  const _StageItem({required this.stage, required this.isLast});

  (Color, Color, IconData) get _props {
    switch (stage.status) {
      case StageStatus.done:       return (AppColors.success,  AppColors.successBg,  Icons.check_rounded);
      case StageStatus.inProgress: return (AppColors.primary,   AppColors.primaryBg,   Icons.arrow_forward_rounded);
      case StageStatus.pending:    return (AppColors.text4,    AppColors.bg,          Icons.circle_outlined);
    }
  }
        ...
        Column(children: [
          ...
          if (!isLast)
            Expanded(child: Container(width: 1.5,
                color: stage.status == StageStatus.done ? AppColors.successBorder : AppColors.border)),
        ]),
        const SizedBox(width: AppDimensions.s12),
        Expanded(
          child: Padding(
            ...
            child: Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(stage.name, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600,
                    color: stage.status == StageStatus.pending ? AppColors.text3 : AppColors.textPrimary)),
                if (stage.time != null)
                  Text(stage.time!, style: const TextStyle(fontSize: 11, color: AppColors.text3)),
              ])),
              if (stage.status == StageStatus.inProgress)
                StatusPill(label: 'Active', bg: AppColors.primaryBg, fg: AppColors.primary)
              else if (stage.status == StageStatus.done)
                StatusPill(label: 'Done', bg: AppColors.successBg, fg: AppColors.success),
            ]),
          ),
        ),
      ]),
