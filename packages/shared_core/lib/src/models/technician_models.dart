...
class AttendanceResponse {
  ...
  factory AttendanceResponse.fromJson(Map<String, dynamic> j) => AttendanceResponse(
    status: j['status'] as String? ?? 'notPunchedIn',
...
class AssignedJobResponse {
  ...
  factory AssignedJobResponse.fromJson(Map<String, dynamic> j) => AssignedJobResponse(
    ...
    plateNumber: (j['plateNumber'] ?? j['registrationNumber'] ?? j['plate'] ?? '').toString(),
...
class TechnicianJobResponse {
  ...
  factory TechnicianJobResponse.fromJson(Map<String, dynamic> j) => TechnicianJobResponse(
    jobCardNo: j['jobCardNo'] as String? ?? '',
    dateOfWork: j['dateOfWork'] as String? ?? '',
    startTime: j['startTime'] as String? ?? '',
    vehicleBrand: (j['vehicleBrand'] ?? j['make'] ?? '').toString(),
    vehicleModel: (j['vehicleModel'] ?? j['model'] ?? '').toString(),
    plateNumber: (j['plateNumber'] ?? j['registrationNumber'] ?? j['plate'] ?? '').toString(),
    customerName: j['customerName'] as String? ?? '',
    customerPhone: j['customerPhone'] as String? ?? '',
    status: j['status'] as String? ?? 'pending',
    tasks: (j['tasks'] as List<dynamic>?)?.map((e) => TaskResponse.fromJson(e)).toList() ?? [],
...
class ProductivityResponse {
  ...
  factory ProductivityResponse.fromJson(Map<String, dynamic> j) => ProductivityResponse(
    assignedJobs: j['assignedJobs'] as int? ?? 0,
    inProgress: j['inProgress'] as int? ?? 0,
    completedToday: j['completedToday'] as int? ?? 0,
    efficiency: j['efficiency'] as int? ?? 0,
    avgTimePerJob: j['avgTimePerJob'] as String? ?? '',
    totalHoursWorked: j['totalHoursWorked'] as String? ?? '',
  );
}
