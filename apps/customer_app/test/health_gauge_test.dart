...
void main() {
  ...
  testWidgets('health gauge renders the real score, not a hardcoded 92%', (
    ...
    expect(find.textContaining('Next scheduled service due'), findsOneWidget);
  });
  ...
  testWidgets('health gauge shows honest empty state without a due date', (
    ...
    expect(find.text('95% OPTIMAL'), findsOneWidget);
    expect(find.text('No pending service milestone on record'), findsOneWidget);
  });

  testWidgets('health gauge does not fabricate a score before assessment', (
    tester,
  ) async {
    await tester.pumpWidget(
      wrap(
