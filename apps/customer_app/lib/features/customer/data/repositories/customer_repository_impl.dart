...
class CustomerRepositoryImpl implements CustomerRepository {
  ...
  Future<CustomerServiceEntity> getActiveService() async {
    try {
      ...
      final entity = CustomerServiceEntity(
        ...
        service: r.service,
        started: r.started,
        estCompletion: r.estCompletion,
        progressPercent: r.progressPercent,
        currentStage: r.currentStage,
        technicianName: r.technicianName,
        stages: r.stages
            .map(
              (s) => ServiceStageEntity(
                name: s.name,
                time: s.time,
  ...
  void _cacheProfile(CustomerEntity e) => _box.put(_profileKey, {
    'name': e.name,
  ...
  void _cacheBookings(List<CustomerBookingEntity> list) =>
      _box.put(_bookingsKey, list.map((b) => b.toJson()).toList());
  List<CustomerBookingEntity> _loadCachedBookings() {
    final list = _box.get(_bookingsKey) as List?;
    if (list == null) return [];
    return list
        .map(
          (m) => CustomerBookingEntity.fromJson(
            Map<String, dynamic>.from(m as Map),
          ),
        )
        .toList();
  ...
  void _cacheNotifications(List<CustomerNotificationEntity> list) =>
      _box.put(_notifsKey, list.map((n) => n.toJson()).toList());
  List<CustomerNotificationEntity> _loadCachedNotifications() {
    final list = _box.get(_notifsKey) as List?;
    if (list == null) return [];
    return list
        .map(
          (m) => CustomerNotificationEntity.fromJson(
            Map<String, dynamic>.from(m as Map),
          ),
        )
        .toList();
  ...
  void _cacheService(CustomerServiceEntity e) =>
      _box.put(_serviceKey, e.toJson());
