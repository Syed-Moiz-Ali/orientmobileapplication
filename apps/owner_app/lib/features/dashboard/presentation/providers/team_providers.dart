...
class BranchOption {
  ...
  factory BranchOption.fromJson(Map<String, dynamic> json) =>
      BranchOption(id: '${json['id']}', name: json['name'] as String? ?? '');
}
...
class TeamNotifier extends Notifier<TeamState> {
  ...
  Future<void> load() async {
    try {
      ...
      final branchList = (await _client.get<List<dynamic>>(
      ...
      state = state.copyWith(
        ...
        staff: list
            .map(
              (e) => StaffMember.fromJson(Map<String, dynamic>.from(e as Map)),
            )
            .toList(),
        branches: branchList
            .map(
              (e) => BranchOption.fromJson(Map<String, dynamic>.from(e as Map)),
            )
            .where((branch) => branch.id != 'null' && branch.name.isNotEmpty)
            .toList(),
      );
    } catch (e, st) {
      ref
  ...
  Future<String?> addMember(Map<String, dynamic> payload) async {
    try {
      ...
      ref
          .read(loggerProvider)
          .e('Failed to add staff', error: e, stackTrace: st);
      final message = e is ValidationException
          ? e.message
          : 'Could not add staff';
      return message;
    }
  ...
  Future<void> toggleActive(StaffMember member) async {
    try {
      if (member.isActive) {
        (await _client.put(
          ApiEndpoints.ownerTeamDeactivate(member.id),
        )).unwrapOrThrow();
        state = state.copyWith(
          staff: state.staff
        ...
        (await _client.put(
          ApiEndpoints.ownerTeamById(member.id),
          data: {'isActive': true},
        )).unwrapOrThrow();
        state = state.copyWith(
          staff: state.staff
