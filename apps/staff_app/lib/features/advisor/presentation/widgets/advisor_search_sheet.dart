...
class _AdvisorSearchSheetState extends ConsumerState<AdvisorSearchSheet> {
        ...
        child: Column(
          ...
          children: [
            ...
            Align(
              ...
              child: Text('Search', style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w700, color: AppColors.textPrimary, letterSpacing: 0.5)),
            ),
            const SizedBox(height: 14),
            TextField(
              controller: _ctrl,
              ...
              decoration: InputDecoration(
                ...
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(AppDimensions.r12), borderSide: const BorderSide(color: AppColors.line)),
                enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(AppDimensions.r12), borderSide: const BorderSide(color: AppColors.line)),
                focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(AppDimensions.r12), borderSide: const BorderSide(color: AppColors.accent, width: 1.5)),
            ...
            if (_query.isNotEmpty) ...[
              ...
              if (!hasResults)
                ...
                if (cardResults.isNotEmpty) ...[
                  ...
                  ...cardResults.map((j) => _resultItem(
                    Icons.assignment_rounded, AppColors.accent,
                    j.customerName, '${j.vehicleInfo}  \u00b7  ${j.id}',
                    () { Navigator.pop(context); context.push(AppRoutes.advisorDashboard); },
                  )),
                ...
                if (customerResults.isNotEmpty) ...[
                  ...
                  const SizedBox(height: 6),
                  ...customerResults.map((c) => _resultItem(
                ...
                if (approvalResults.isNotEmpty) ...[
                  ...
                  const SizedBox(height: 6),
                  ...approvalResults.map((a) => _resultItem(
                    Icons.thumb_up_rounded, AppColors.warning,
                    a.customerName, '${a.estimateId}  \u00b7  AED ${a.amount.toStringAsFixed(0)}',
                    () => Navigator.pop(context),
                  )),
                ],
              ],
  ...
  Widget _sectionHeader(String label, int count) => Row(
    children: [
      Text('$label ($count)', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.text2, letterSpacing: 0.3)),
    ],
  );
