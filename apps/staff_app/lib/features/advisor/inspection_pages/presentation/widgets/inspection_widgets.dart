...
({Color color, Color bg, String label}) statusColors(ItemStatus s) {
  switch (s) {
    case ItemStatus.good:
      return (color: AppColors.primary, bg: AppColors.primaryBg, label: 'Good');
    case ItemStatus.fair:
      return (color: AppColors.warning, bg: AppColors.warningBg, label: 'Fair');
    case ItemStatus.poor:
      return (color: AppColors.danger, bg: AppColors.dangerBg, label: 'Poor');
  }
}
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.all(Radius.circular(AppDimensions.r7)),
            border: Border.all(
              color: active ? AppColors.primary : color.withValues(alpha: 0.6),
              width: 1.5,
            ),
