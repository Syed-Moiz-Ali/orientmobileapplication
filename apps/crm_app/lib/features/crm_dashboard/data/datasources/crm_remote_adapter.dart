...
class CrmRemoteAdapter implements CrmDataSource {
  ...
  @override
  List<CrmKpiEntity> get kpis => _kpis;
  @override
  List<CrmChannelEntity> get channels => _channels;
  @override
  List<CrmTrendPoint> get conversionTrend => _conversionTrend;
  @override
  List<SalespersonPerf> get salespersonPerf => _salespersonPerf;
  @override
  List<ResponseTimeBucket> get responseTimeBuckets => _responseTimeBuckets;
  @override
  List<LeadSourceSlice> get leadSources => _leadSources;
  @override
  List<CrmKeyMetric> get keyMetrics => _keyMetrics;
  @override
  List<IntegrationEntity> get integrations => _integrations;
  @override
  List<SalesTeamMember> get salesTeam => _salesTeam;
  @override
  List<ConversationEntity> get conversations => _conversations;
  @override
  List<CrmLeadEntity> getLeads() => _leads;
  @override
  List<CrmTaskEntity> getTasks() => _tasks;
  ...
  Future<void> loadAll() async {
    ...
    final results = await Future.wait([
      _remote.getKpis(),
      _remote.getChannels(),
      _remote.getConversionTrend(),
      _remote.getSalespersonPerformance(),
      _remote.getResponseTimes(),
      _remote.getLeadSources(),
      _remote.getKeyMetrics(),
      _remote.getIntegrations(),
      _remote.getSalesTeam(),
    ...
    _conversionTrend = (results[2] as List)
        .map(
          (e) => CrmTrendPoint(
        ...
        .toList();
    _salespersonPerf = (results[3] as List)
        .map((e) => SalespersonPerf((e as SalespersonPerfResponse).name, e.leads.toDouble(), e.won.toDouble()))
        .toList();
    _responseTimeBuckets = (results[4] as List)
        .map((e) => ResponseTimeBucket((e as ResponseTimeResponse).label, e.count.toDouble()))
        .toList();
    _leadSources = (results[5] as List)
        .asMap()
        .entries
        .map(
          (e) => LeadSourceSlice(
            (e.value as LeadSourceResponse).label,
            e.value.percent.toDouble(),
  ...
  List<CrmChannelEntity> _mapChannels(List l) {
    return l
        ...
        .map(
          (e) => CrmChannelEntity(
            label: (e.value as ChannelResponse).name,
            value: (e.value as ChannelResponse).count.toString(),
  ...
  List<CrmKeyMetric> _mapKeyMetrics(KeyMetricResponse km) {
    return [
      CrmKeyMetric(
        label: 'Win Rate',
        value: '${km.winRate.toStringAsFixed(1)}%',
        sub: '',
        up: true,
      ...
      CrmKeyMetric(
        ...
        value: km.avgResponseTime.isEmpty ? '--' : km.avgResponseTime,
        sub: '',
        up: true,
      ...
      CrmKeyMetric(
        ...
        value: km.satisfaction > 0 ? km.satisfaction.toStringAsFixed(1) : '--',
        sub: '',
        up: true,
  ...
  List<SalesTeamMember> _mapSalesTeam(List l) {
    return l
        .map(
          (e) => SalesTeamMember(
            name: (e as SalesTeamResponse).name,
            role: e.role,
            leadsHandled: e.leadsHandled,
            wonDeals: e.wonDeals,
            revenue: e.revenue,
            winRate: e.winRate,
          ),
    ...
    return l
        .map(
          (e) => ConversationEntity(
            id: (e as ConversationResponse).id,
            customerName: e.customerName,
            lastMessage: e.lastMessage,
            time: e.time,
            channel: e.channel,
            channelColor: _channelColor(e.channel),
            unread: e.unread,
            status: e.status,
          ),
        )
  ...
  List<CrmLeadEntity> _mapLeads(List l) {
    return l.asMap().entries.map((e) {
      ...
      return CrmLeadEntity(
        id: ld.id,
        sno: e.key + 1,
        leadNumber: ld.leadNumber,
        customerName: ld.customerName,
        phone: ld.phone,
        email: ld.email,
        source: ld.source,
        sourceColor: _sourceColor(ld.source),
        assignedTo: ld.assignedTo,
        status: ld.status,
        statusColor: _statusColor(ld.status),
        lastActivity: ld.lastActivity,
        notes: ld.notes,
        leadValue: ld.leadValue,
        followUpDate: ld.followUpDate,
      );
    }).toList();
  }
  ...
  List<TeamMemberEntity> _mapTeamMembers(List l) {
    return l
        .map(
          (e) => TeamMemberEntity(
            name: (e as TeamMemberResponse).name,
            role: e.role,
  ...
  List<LeadActivityEntity> _mapActivities(List l) {
    return l
        .map(
          (e) => LeadActivityEntity(
            id: (e as LeadActivityResponse).id,
            action: e.action,
            detail: e.detail,
  ...
  Color _channelColor(String c) => switch (c) {
    ...
    'SMS' => AppColors.purpleAccent,
    'Website' => AppColors.cyanBright,
    _ => AppColors.amber500,
  };
  Color _sourceColor(String s) => _channelColor(s);
  Color _statusColor(String s) => switch (s) {
    'WON' => AppColors.greenAccent,
    'ACTIVE' => AppColors.greenAccent,
    'UNANSWERED' => AppColors.warning,
    _ => AppColors.red500,
  };
  Color _priorityColor(String p) => switch (p) {
    'HIGH' => AppColors.red500,
    'MEDIUM' => AppColors.warning,
    _ => AppColors.greenAccent,
  };
}
